...
- **Maßnahmen-Vorschläge (NEU):** „💡 Vorschläge"-Bereich im Maßnahmen-Tab. Katalog aus `benchmarks.js` (quick_wins/massnahmen je Branche) + KI über `callOpenRouterDirect`/`/api/ki-consulting` (JSON-Parsing `parseKIVorschlaege` + Text-Fallback); „+ Übernehmen" → `POST /api/massnahmen`. Kein DB-/API-Umbau. (Spec/Plan in `docs/superpowers/`.)
